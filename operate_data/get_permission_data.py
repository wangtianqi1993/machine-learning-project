...
def get_permission_data(benign_path, malware_path):
    ...
    """
    feature_vector = []
    with open(benign_path, "r") as f1:
        benign = json.load(f1)
        print len(benign)
        for item in benign:
            temp = [0]*len_base
            for per in benign[item]:
                per = per.split(".")[-1]
                if per in base_permission:
                    temp[base_permission.index(per)] = 1
            temp.append(0)
            feature_vector.append(temp)

    with open(malware_path, "r") as f2:
        malware = json.load(f2)
        print len(malware)
        for item in malware:
            temp = [0]*len_base
            for per in malware[item]:
                per = per.split(".")[-1]
                if per in base_permission:
                    temp[base_permission.index(per)] = 1
            temp.append(1)
